class FornecedorsController < ConfigsController 
  before_action :set_fornecedor, only: [:show, :edit, :update, :destroy]

  # GET /fornecedors
  # GET /fornecedors.json
  def index
    @fornecedors = Fornecedor.usuario(current_user).order(:nome)
  end
  ...
  def create
    ...
    respond_to do |format|
      if @fornecedor.save
        format.html { redirect_to fornecedors_path, notice: 'Fornecedor registrado com Sucesso.' }
  ...
  private
    ...
    def fornecedor_params
      params.require(:fornecedor).permit(:nome, :endereco, :cidade, :telefone, :documento, :ativo).merge(user_id: current_user.id)
    end
end

prawn_document(page_layout: :portrait)  do |pdf|
  ...
  pdf.text "À Pagar #{@anos_atual[0].ano}", size: 10, style: :bold, align: :center 
  pdf.stroke_horizontal_rule

  if @lancamentos_apagar.blank?
  pdf.move_down 180
  pdf.text 'SEM REGISTROS', size: 30, :style => :bold, :align => :center 
  else
  pdf.move_down 20

 dados = @lancamentos_apagar.map do |f|
    
  if f.valor <= f.pagamentos.sum(:valor)
  else
    [data(f.data),
      "#{f.descricao} - #{f.parcela}/#{ f.number}",
      valor(f.valor),
...
headers = ["Lancamento", "Descrição", "Valor", "Pago", "À Pagar", "Situação"]
